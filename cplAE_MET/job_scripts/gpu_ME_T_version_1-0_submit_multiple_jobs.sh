...
aug_dec=1
exp_name_pre="triple_mode_version_1.0"

c=0
for nf in {0..9};do
    c=$((c+1))
    jobname="cpl_ME_T"$cpl_ME_T"_nfold"$nf
    exp_name=$exp_name_pre'/'$model_id'_aT_'${alpha_T}'_aE_'${alpha_E}'_aM_'${alpha_M}'_asd_'${alpha_sd}'_aME_'${alpha_ME}'_lmd_ME_T_'${lambda_ME_T}'_lmd_E_T_'${lambda_E_T}'_lmd_M_T_'${lambda_M_T}'_lmd_tune_ME_T_'${lambda_tune_ME_T}'_aug_dec_'${aug_dec}
    echo $exp_name
    ...
    echo 'python -m train_T_EM_AE_version_1 --model_id '${model_id}'  --exp_name '${exp_name}' --alpha_T '${alpha_T}' --alpha_E '${alpha_E}' --alpha_M '${alpha_M}' --alpha_sd  '${alpha_sd}' --alpha_ME '${alpha_ME}' --lambda_ME_T '${lambda_ME_T}' --lambda_M_T '${lambda_M_T}' --lambda_E_T '${lambda_E_T}' --lambda_tune_ME_T '${lambda_tune_ME_T}'  --latent_dim '${latent_dim}' --n_epochs 50000 --n_fold '${nf} --augment_decoders ${aug_dec}''>>subjob.sh
    echo '...'
